...
// // 	o, err := ykoath.New(30 * time.Second)
// // 	require.NoError(err)
...
// // 	p, err := sw.New(key, sha256.New, 30*time.Second)
// // 	require.NoError(err)

// // 	now := time.Now()

// // 	hashOATH := p.CalculateTOTP(now)
...
// // 	hashYKOATH, digits, err := o.CalculateTOTP("gotest", now)
// // 	require.NoError(err)
